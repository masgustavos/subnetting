...
- Fetch `<origin>/llms.txt` before choosing any method, with your HTTP tool or from inside the tab (`async () => (await fetch('/llms.txt')).text()`). It is generated from the deployed source and lists every method, signature, mode rule, recipe, keyboard shortcut and DOM landmark. `window.subnet.help()` is the short index of the same list.
...
Any tool that evaluates JavaScript in the page works, known to this skill or not. The contract: pass a function, `await` async facade calls inside it, and return `JSON.stringify(result)`. Every facade method returns `{ ok: true, data }` or `{ ok: false, error, code }` and does not throw.
...
2. Fetch the contract (see above) and read it.
...
| Route            | Purpose                                                  |
| ---------------- | -------------------------------------------------------- |
| `/`              | Main app on the scratch network (browser storage only)   |
...
- Ask the mode's discovery questions in one consolidated turn, with your harness's structured-question tool when it has one, otherwise as numbered questions with a recommended default and a reason each. AWS mode follows [references/aws-network-design.md](references/aws-network-design.md) Q0 to Q8; Generic mode asks only parent CIDR, child count (power of two) and sizing or tier mix. For naming choices, show side-by-side ASCII previews of the variants.
- Propose the resulting plan as a preview: parent CIDR, every child CIDR, per-VPC-type per-AZ shape, agreed names per level, reserved remainder, and the facade calls you intend to issue. State which defaults you applied and why.
- A fully specified request skips the questions but still gets a one-line preview ("I'll create <plan>; reply confirm to proceed").
- Wait for an explicit go-ahead. "Hmm", "wait", a follow-up question or more spec is not consent: re-ask what changed and re-preview.
...
- The gate is keyed on the route, not the origin. It always applies on `/network/<id>`, because a dev server on a cloud route syncs to the real account as well. It applies on `/` unless the user, or the project's instructions, called that network disposable. `/shared/<slug>` is read-only, so nothing there can be destroyed.
- Confirm, and wait for a clear go-ahead, before: `deleteBlock` on a top-level container (account, region, VPC); `clearChildren` on the root or any non-empty top-level container; `setMode` to a mode that would reset or reshape the structure; `changeCIDR` on the root or any container with downstream subnets; `pasteOverChildren` onto a non-empty container; the triple-`Escape` reset.
...
- Ask in one line: name the operation, name the consequence, give a "reply X to proceed" path. That reads as professional, not obstructive.
...
- `assertSchema` may carry a check wherever its `/llms.txt` entry shows it covers that check; whatever it does not cover is asserted per block from `listBlocks`.
- Report counts of VPCs, AZs and subnets checked plus the list of errors, and declare complete only at zero errors; otherwise show the list and propose a fix (targeted mutations, or `clearChildren` plus a rebuild under a bundled Gate 2 confirmation).
